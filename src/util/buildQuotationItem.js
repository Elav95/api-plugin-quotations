...
/**
 * @summary Builds an quotation item
 * @param {Object} context an object containing the per-request state
 * @param {String} currencyCode The quotation currency code
 * @param {Object} inputItem Quotation item input. See schema.
 * @returns {Promise<Object>} An quotation item, matching the schema needed for insertion in the Quotations collection
 */
export default async function buildQuotationItem(context, { currencyCode, inputItem }) {
  ...
  const inventoryInfo = await context.queries.inventoryForProductConfiguration(context, {
    fields: ["canBackquotation", "inventoryAvailableToSell"],
    productConfiguration: {
      ...productConfiguration,
      isSellable: true
    },
  ...
  if (!inventoryInfo.canBackquotation && (quantity > inventoryInfo.inventoryAvailableToSell)) {
    throw new ReactionError("invalid-quotation-quantity", `Quantity quotationed is more than available inventory for  "${chosenVariant.title}"`);
  }
  ...
  const newItem = {
    ...
    workflow: { status: "new", workflow: ["coreQuotationWorkflow/created", "coreItemWorkflow/removedFromInventoryAvailableToSell"] }
  };

  for (const func of context.getFunctionsOfType("mutateNewQuotationItemBeforeCreate")) {
